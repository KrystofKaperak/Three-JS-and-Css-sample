...
THREE.SearchControls = function ( camera, domElement, self, options) {
// Solves negative modulo JavaScript bug
	...
	this.tiltMode = false;
	this.cameraLookdownAngle = (1/4)*Math.PI ;

	function cameraPositionUpdate(){		
		// var thisPosition =  getRadialPlot(scope.cameraPosition.xzRad, scope.cameraPosition.yRot, scope.cameraPosition.yPos + Math.tan(scope.cameraLookdownAngle)*(scope.cameraPosition.xzRad - self.cardRadius)/9 ).clone();
		...
		if (scope.target === null) {
			// scope.camera.lookAt(new THREE.Vector3(0,thisPosition.y  -scope.cameraPosition.xzRad/9, 0) ); 			
...
THREE.SearchControls.prototype = Object.create( THREE.EventDispatcher.prototype );
